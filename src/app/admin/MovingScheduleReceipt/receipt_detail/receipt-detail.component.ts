...
export class MovingScheduleReceiptDetailComponent implements OnInit, OnDestroy {
    ...
    this.subscriptions.push(
      this.receiptState.subscribe((state) => {
        if (state) {
          ...
          this.getTour(state.totalReceipt?.scheduleId);

          console.log(state.totalReceipt?.scheduleId);
  ...
  formReset(): void {
    this.editformGroup_info.setValue({
      ...
      scheduleId: this.receipt.scheduleId ?? "",
  ...
  formSubmit_edit_info(): void {
    ...
    if (this.editformGroup_info.valid){
      const payload: FullReceipt = {
        ...
        scheduleId: this.editformGroup_info.value.scheduleId,
        scheduleType: 1,
  ...
  selectChangeReceipt($event: any): any {
    if ($event.data) {
      this.editformGroup_info.controls["scheduleId"].setValue(
        $event.data.idList[0]
      );
    }
  }
